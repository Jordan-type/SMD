...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String KEY_ALARMS_PILL_CONTAINER = "container";
    private static final String KEY_ALARMS_PILL_REMAINING         = "remaining";
    ...
    private static final String CREATE_PILL_TABLE =
            "create table " + PILL_TABLE + "("
                    ...
                    + KEY_CONTAINER + " text not null,"
                    + KEY_REMAINING + " text not null"+ ")";
    ...
    private static final String CREATE_PhoneNumber_TABLE =
...
// ############################## create methods ###################################### //
    ...
    public String getPhoneNumber()
    {
        ...
        String dbphonenumber = "select * from "
                + PhoneNumber        ;

        Cursor c = db.rawQuery(dbphonenumber, null);
        if(c!=null && c.getCount()>0 && c.moveToFirst()) {
            return c.getString(c.getColumnIndex(KEY_PhoneNumber));
    ...
    {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("UPDATE "           + PhoneNumber +
                " SET " + KEY_PhoneNumber + " = " + number +
                " WHERE " + KEY_ROWID + " = " + "1");
    }
    public long createPill(Pill pill) {
        ...
        values.put(KEY_CONTAINER,pill.getContainerName());
        values.put(KEY_REMAINING,pill.getRemainingPills());

        long pill_id = db.insert(PILL_TABLE, null, values);

        //String pillid = pillreference.push().getKey();


       //pillreference.child(Long.toString(pill_id)).setValue(pill);
...
// ############################# get methods ####################################### //
    ...
    public Pill getPillByName(String pillName) {
        ...
        if (c.moveToFirst() && c.getCount() >= 1) {
            ...
            pill.setContainerName(c.getString(c.getColumnIndex(KEY_CONTAINER)));
            pill.setRemainingPills(c.getString(c.getColumnIndex(KEY_REMAINING)));
            c.close();
        }
        return pill;
    }
    ...
    public List<Pill> getAllPills() {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                p.setContainerName(c.getString(c.getColumnIndex(KEY_CONTAINER)));
                p.setRemainingPills(c.getString(c.getColumnIndex(KEY_REMAINING)));
                pills.add(p);
            } while (c.moveToNext());
        }
        c.close();
    ...
    public List<Alarm> getallAlarms() {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                al.setMinute(c.getInt(c.getColumnIndex(KEY_MINUTE)));
                al.setPillName(c.getString(c.getColumnIndex(KEY_ALARMS_PILL_NAME)));
                al.setDay(c.getInt(c.getColumnIndex(KEY_DAY_WEEK)));
    ...
    public List<Alarmlinks> getallalarmlinks()
    {
        List<Alarmlinks> link = new ArrayList<Alarmlinks>();
        ...
        if (c.moveToFirst()) {
            do {
                Alarmlinks a = new Alarmlinks();
                a.setId(c.getInt(c.getColumnIndex(KEY_ROWID)));
                a.setAlarm_id(c.getInt(c.getColumnIndex("alarm_id")));
                a.setPill_id(c.getInt(c.getColumnIndex("pill_id")));
...
// ############################### delete methods##################################### //
    ...
    public void updatePillRemaining(String pill_id,String Remaining)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("UPDATE "           + PILL_TABLE +
                " SET " + KEY_REMAINING + " = " + Remaining +
                " WHERE " + KEY_ROWID + " = " + pill_id);
    }

    public void updatePillContainer(String pill_id,String container)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("UPDATE "           + PILL_TABLE +
                " SET " + KEY_CONTAINER + " = " + container +
                " WHERE " + KEY_ROWID + " = " + pill_id);
    }

    public void insertpilltable(List<Pill> pills)
    {
        SQLiteDatabase db = this.getWritableDatabase();

        db.execSQL("Delete From pills");
        for(Pill pill : pills)
        {
            createPill(pill);
        }
    ...
    public void inserthisttable(List<History> allhistory) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void insertalarms(List<Alarm> allalarms)
    {
        SQLiteDatabase db = this.getWritableDatabase();

        db.execSQL("Delete From alarms");
        for(Alarm al : allalarms)
        {
            ContentValues values = new ContentValues();
            values.put(KEY_ROWID, al.getId());
            values.put(KEY_HOUR, al.getHour());
            values.put(KEY_MINUTE, al.getMinute());
            values.put(KEY_DAY_WEEK, al.getDay());
            values.put(KEY_ALARMS_PILL_NAME, al.getPillName());
    ...
    public void insertalarmslink(List<Alarmlinks> allalarmslink)
    {
        SQLiteDatabase db = this.getWritableDatabase();

        db.execSQL("Delete From pill_alarm");
        for(Alarmlinks al: allalarmslink)
        {
            ContentValues values = new ContentValues();
            values.put(KEY_ROWID, al.getId());
            values.put(KEY_ALARMTABLE_ID, al.getAlarm_id());
            values.put(KEY_PILLTABLE_ID, al.getPill_id());

            /** Insert row */
            db.insert(PILL_ALARM_LINKS, null, values);
        }
    }

    public void insertphonenuber(String phonenumber) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public List<user> getallusers()
    {
        ...
        String dbPills = "SELECT * FROM users" ;
